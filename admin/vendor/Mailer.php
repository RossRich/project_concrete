...
class Mailer {
    ...
    public function mail($to, $subject, $message, $options = []) {
        ...
        /*if (isset($options['from'])) {
          	$message->setFrom($from = $options['from'], isset($options['fromName']) ? $options['fromName'] : false);
        } else {
            if($this->transport == 'smtp') {
                $message->setFrom($this->options['from'], $this->options['fromName']);
            } else {
              	$message->setFrom('mailer@'.(isset($_SERVER["SERVER_NAME"]) ? $_SERVER["SERVER_NAME"] : 'localhost'));
            }
        }*/
