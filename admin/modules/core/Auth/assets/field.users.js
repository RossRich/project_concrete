(function($){
    ...
    angular.module('cockpit.fields').run(['Contentfields', function(Contentfields) {
        ...
        Contentfields.register('link-user', {
            label: 'Users',
            template: function(model, options) {
                return '<div link-user="'+options.collection+'" ng-model="'+model+'" data-multiple="'+(options.multiple ? 'true':'false')+'">Linking '+options.collection+'</div>';
            }
        });
    ...
    angular.module('cockpit.fields').directive("linkUser", ['$timeout', '$http', function($timeout, $http) {
        ...
        Field = function($element, options, model, scope) {
            ...
            this.options    = $.extend({multiple: false, user: null}, options);
            this.model      = model;
                ...
                if (!this.value) {
                    ...
                    this.element.html([
                        '<div class="uk-placeholder uk-text-center">',
                            '<strong class="uk-text-small">'+this.user.name+'</strong>',
