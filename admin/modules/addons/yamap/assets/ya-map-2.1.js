...
angular.module('cockpit.directives').
	...
	// TODO: update templateFactory in yaControl and yaCluster as in yaGeoObject and delete this service
    ...
    directive('yaTemplateLayout',['mapApiLoad', function(mapApiLoad){
        return{
            restrict:'E',
            scope:{
                overrides:'=yaOverrides',
              	templateFactory:'=yaTemplateFactory',
            },
            compile: function(tElement) {
                ...
                return function(scope,elm,attrs){
                    ...
                    if (!scope.templateFactory) scope.templateFactory = new templateFactory;
                  	scope.me = "template";
                    if(!attrs.yaKey){
                        throw new Error('not require attribute "key"');
                    }
                    var key = attrs.yaKey;
                    scope.templateFactory.create(key, html, scope.overrides);
                };
            }
        };
    }]).
    ...
    directive('yaGeoObject',['GEOMETRY_TYPES','yaSubscriber','$parse',function(GEOMETRY_TYPES,yaSubscriber,$parse){
        return {
            restrict:'E',
            ...
            link:function(scope,elm,attrs,ctrls){
                var ctrl = ctrls[2] || ctrls[1] || ctrls[0],
                    obj;
                var options = attrs.yaOptions ? scope.$eval(attrs.yaOptions) : undefined;
                if(options && options.balloonContentLayout && scope.templateFactory){
                    options.balloonContentLayout = scope.templateFactory.get(options.balloonContentLayout);
                }
                if(options && options.iconLayout && scope.templateFactory){
                    options.iconLayout = scope.templateFactory.get(options.iconLayout);
                }
