<?
$phone = cockpit('regions:region_field', 'Контакты', 'phone', 'value');
$phone2 = cockpit('regions:region_field', 'Контакты', 'phone2', 'value');
$phone3 = cockpit('regions:region_field', 'Контакты', 'phone3', 'value');
$email = cockpit('regions:region_field', 'Контакты', 'email', 'value');
$address = cockpit('regions:region_field', 'Контакты', 'address', 'value');
$address2 = cockpit('regions:region_field', 'Контакты', 'address2', 'value');
$social["vk"] = cockpit('regions:region_field', 'Контакты', 'vk', 'value');
$social["ok"] = cockpit('regions:region_field', 'Контакты', 'ok', 'value');
$social["fb"] = cockpit('regions:region_field', 'Контакты', 'fb', 'value');
?>