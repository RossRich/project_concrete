<div class="advantages_full">
    <div class="uk-container2 uk-container-center advantages_new">
        <h3 class="uk-text-center">Доверие, заслуженное качеством и технологиями</h3>
        <ul class="uk-grid uk-grid-width-small-1-1 uk-grid-width-medium-1-2 uk-grid-width-large-1-4" data-uk-grid-margin="{cls:'mt35'}">
            <li>
                <div class="uk-panel uk-panel-box uk-clearfix plus_panel">
                    <div class="plus_logo">
                        <img src="/images/plus1.svg" alt="">
                    </div>
                    <div class="plus_text">
                        <h5>Современное оборудование</h5>
                    </div>
                </div>
            </li>
            <li>
                <div class="uk-panel uk-panel-box uk-clearfix plus_panel">
                    <div class="plus_logo">
                        <img src="/images/plus2.svg" alt="">
                    </div>
                    <div class="plus_text">
                        <h5>Производственная лаборатория</h5>
                    </div>
                </div>
            </li>
            <li>
                <div class="uk-panel uk-panel-box uk-clearfix plus_panel">
                    <div class="plus_logo">
                        <img src="/images/plus3.svg" alt="">
                    </div>
                    <div class="plus_text">
                        <h5>Собственный парк спецтехники</h5>
                    </div>
                </div>
            </li>
            <li>
                <div class="uk-panel uk-panel-box uk-clearfix plus_panel">
                    <div class="plus_logo">
                        <img src="/images/plus4.svg" alt="">
                    </div>
                    <div class="plus_text">
                        <h5>Гарантия низкой цены</h5>
                    </div>
                </div>
            </li>
        </ul>
    </div>
</div>

<!--<div class="advantages_full">-->
<!--            <div class="uk-container2 uk-container-center advantages">-->
<!--                <h3 class="uk-text-center">Доверие, заслуженное качеством и технологиями</h3>-->
<!--                <ul class="uk-grid uk-grid-width-small-1-1 uk-grid-width-medium-1-2 uk-grid-width-large-1-2" data-uk-grid-margin="{cls:'mt35'}">-->
<!--                    <li>-->
<!--                        <div class="uk-panel uk-panel-box uk-clearfix plus_panel">-->
<!--                            <div class="plus_logo">-->
<!--                                <img src="/images/plus1.png" alt="">-->
<!--                            </div>-->
<!--                            <div class="plus_text">-->
<!--                                <h4>СОВРЕМЕННОЕ ОБОРУДОВАНИЕ</h4>-->
<!--                                <p>Глубокий цикл очистки компонентов бетона</p>-->
<!--                                <p>Точная система весового контроля загрузки миксеров</p>-->
<!--                                <p>Полное соответствие производства продукта по ГОСТу</p>-->
<!--                            </div>-->
<!--                        </div>-->
<!--                    </li>-->
<!--                    <li>-->
<!--                        <div class="uk-panel uk-panel-box uk-clearfix plus_panel">-->
<!--                            <div class="plus_logo">-->
<!--                                <img src="/images/plus2.png" alt="">-->
<!--                            </div>-->
<!--                            <div class="plus_text">-->
<!--                                <h4>СОБСТВЕННЫЙ ПАРК СПЕЦТЕХНИКИ</h4>-->
<!--                                <p>GPS контроль доставки в каждой машине</p>-->
<!--                                <p>Квалифицированные водители с допусками</p>-->
<!--                                <p>Собственный парк спецтехники: более 15 единиц</p>-->
<!--                            </div>-->
<!--                        </div>-->
<!--                    </li>-->
<!--                    <li>-->
<!--                        <div class="uk-panel uk-panel-box uk-clearfix plus_panel">-->
<!--                            <div class="plus_logo">-->
<!--                                <img src="images/plus3.png" alt="">-->
<!--                            </div>-->
<!--                            <div class="plus_text">-->
<!--                                <h4>СОБСТВЕННАЯ ЛАБОРАТОРИЯ ЗАВОДА</h4>-->
<!--                                <p>Предоставление полной технической документации</p>-->
<!--                                <p>Проверка лабораторного образца на прочность</p>-->
<!--                                <p>100% юридическая ответственность за продукт</p>-->
<!--                            </div>-->
<!--                        </div>-->
<!--                    </li>-->
<!--                    <li>-->
<!--                        <div class="uk-panel uk-panel-box uk-clearfix plus_panel">-->
<!--                            <div class="plus_logo">-->
<!--                                <img src="/images/plus4.png" alt="">-->
<!--                            </div>-->
<!--                            <div class="plus_text">-->
<!--                                <h4>ГАРАНТИЯ НИЗКОЙ ЦЕНЫ</h4>-->
<!--                                <p>У нас всегда дешевле на 10%</p>-->
<!--                                <p>Быстрая доставка без простоя на объекте</p>-->
<!--                                <p>Гарантированное качество, за которое мы отвечаем</p>-->
<!--                            </div>-->
<!--                        </div>-->
<!--                    </li>-->
<!--                </ul>-->
<!--            </div>-->
<!--        </div>-->
